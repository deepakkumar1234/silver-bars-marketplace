...
public class LiveOrderBoard {
	...
	public List<OrderSummary> summary() {
		return registeredOrders.stream().collect(groupingBy(Bid::forOrder, mapping(Order::quantity, toList())))
				.entrySet().stream().map(toOrderSummary()).sorted(byTypeAndPrice).collect(toList());
	}
